...
description: >
  ...
  optimizes Defender/NTFS/Git/Power for speed, and writes AGENTS.md so the agent uses grep/sed/awk naturally in future sessions. Use when a
  Windows user wants a better terminal, Unix commands, or has encoding issues.
---

# unix-on-windows — Agent Setup Instructions
...
- If starship already exists → user may have unix-on-windows already, ask before overwriting
...
```powershell
git clone https://github.com/redtidev1918/unix-on-windows.git "$env:USERPROFILE\.unix-on-windows"
cd "$env:USERPROFILE\.unix-on-windows"
```
